...
namespace PalTracker
{
  public class MySqlTimeEntryRepository : ITimeEntryRepository
  {
    ...
    public IEnumerable<TimeEntry> List() => _context.TimeEntryRecords.AsNoTracking().Select(t => t.ToEntity());

    public TimeEntry Update(long id, TimeEntry timeEntry)
    {
      var recordToUpate = timeEntry.ToRecord();
      recordToUpate.Id = id;

      _context.Update(recordToUpate);
      _context.SaveChanges();
    ...
    private TimeEntryRecord FindRecord(long id) => _context.TimeEntryRecords.AsNoTracking().Single(t => t.Id == id);
  }
}
